  ...
  FetchDetail(id:any)
  {
    this.service.GetUserById(id).subscribe((data:any)=>{
          ...
          this.updateForm = this.fb.group({
              ...
              phoneNumber:[this.updateData.phoneNumber,Validators.compose([Validators.required, Validators.pattern(/^[1-9][0-9]{9}$/)])],
              emailAddress:[this.updateData.emailAddress,Validators.compose([Validators.required,Validators.email])],
              password:[this.updateData.password,Validators.compose([Validators.required])],
