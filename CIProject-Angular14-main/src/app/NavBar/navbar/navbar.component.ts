...
export class NavbarComponent implements OnInit {
  ...
  constructor(private service:AdminloginService,private router:Router,public Clientservice:ClientService) { }
  ...
  ngOnInit(): void {
    this.service.getCurrentUser().subscribe((data:any)=>{
      ...
      if(userName != null || data != null){
        ...
        // data == null ? (this.userImage = this.service.imageUrl + '/' + userName.userImage) : (this.userImage = this.service.imageUrl + '/' + userName.userImage);
      }
      
    });
    this.Clientservice.GetUserProfileDetailById(this.loginUserId).subscribe((data:any)=>{
      if(data.result == 1)
        {
          this.userImage = this.Clientservice.imageUrl + '/' + data.data.userImage
        }
    });
