    ...
    public class DALCommon
    {
        ...
        public async Task<List<DropDown>> MissionSkillListAsync()
        {
            return await _cIDbContext.MissionSkill
                ...
                .Select(ms => new DropDown { Value = ms.Id, Text = ms.SkillName }).ToListAsync();
        }
        ...
        public async Task<string> AddUserSkillAsync(UserSkills userSkills)
        {
            try
            {
                UserSkills user =  _cIDbContext.UserSkills.FirstOrDefault(us => us.UserId == userSkills.UserId && !us.IsDeleted);
                if (user == null)
                {
