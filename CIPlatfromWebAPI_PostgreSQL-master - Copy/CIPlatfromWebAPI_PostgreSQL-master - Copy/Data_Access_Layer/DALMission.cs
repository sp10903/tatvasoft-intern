    ...
    public class DALMission
    {
        ...
        public List<DropDown> GetMissionSkillList()
        {
            return _cIDbContext.MissionSkill
                 ...
                 .Select(ms => new DropDown { Value = ms.Id, Text = ms.SkillName }).ToList();
        }
        ...
        {
            var missions = _cIDbContext.Missions
            ...
            try
            {
                clientSideMissionlist = _cIDbContext.Missions
                    ...
                    .Select(m => new Missions
                    {
                        ...
                        MissionSkillName = m.MissionSkillName,
                        ...
                        MissionStatus = m.RegistrationDeadLine < DateTime.UtcNow.AddDays(-1) ? "Closed" : "Available",
                        MissionApplyStatus = _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == userId) ? "Applied" : "Apply",
                        MissionApproveStatus = _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == ma.UserId && ma.Status == true) ? "Approved" : _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == ma.UserId && ma.Status == false && ma.IsDeleted) ? "Denies" : "Applied",
                        MissionDateStatus = m.EndDate <= DateTime.UtcNow.AddDays(-1) ? "MissionEnd" : "MissionRunning",
                        MissionDeadLineStatus = m.RegistrationDeadLine <= DateTime.UtcNow.AddDays(-1) ? "Closed" : "Running",
                    })
                    .ToList();
            }
            catch (Exception)
        ...
        public List<Missions> MissionClientList(SortestData data)
        {
            ...
            var result = _cIDbContext.Missions
                .Where(m => !m.IsDeleted)
                .Select(m => new Missions
                {
                    Id = m.Id,
                    ...
                    MissionApproveStatus = _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == data.UserId && ma.Status == true) ? "Approved" : _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == data.UserId && ma.Status == false && ma.IsDeleted) ? "Denies" : "Applied",                   
                    MissionDateStatus = m.EndDate <= DateTime.UtcNow.AddDays(-1) ? "MissionEnd" : "MissionRunning",
                    MissionDeadLineStatus = m.RegistrationDeadLine <= DateTime.UtcNow.AddDays(-1) ? "Closed" : "Running",
            ...
            if (data.SortestValue == "Newest")
            {
                result = result.OrderBy(m => m.CreatedDate).Reverse().ToList();
            }
            else if (data.SortestValue == "Oldest")
            {
                result = result.OrderBy(m => m.CreatedDate).ToList();
            }
            else if (data.SortestValue == "Lowest available seats")
            {
                result = result.OrderBy(m => m.TotalSheets).ToList();
            }else if (data.SortestValue == "Highest available seats")
            {
                result = result.OrderBy(m => m.TotalSheets).Reverse().ToList();
            ...
            try
            {
                ...
                var missionDetail = _cIDbContext.Missions
                    ...
                    .Select(m => new Missions
                    {
                        ...
                        MissionStatus = m.RegistrationDeadLine < DateTime.UtcNow.AddDays(-1) ? "Closed" : "Available",
                        MissionApplyStatus = _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == userId) ? "Applied" : "Apply",
                    MissionApproveStatus = _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == userId && ma.Status == true) ? "Approved" : _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == userId && ma.Status == false && ma.IsDeleted) ? "Denies" : "Applied",
